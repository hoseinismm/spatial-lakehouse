...
import ir.smh.spatialbricks.decoder.NFSPDecoder;
...
public class NFSP implements UDFRegistry<Geometry,Map<String, Object>>, Serializable {

    SparkSession spark;

    public NFSP(SparkSession spark) {
        this.spark = spark;
    }
    ...
    private  static final StructType GEOMETRY_TYPE =
            DataTypes.createStructType(new StructField[]{
                    ...
                    DataTypes.createStructField("geometries", DataTypes.createArrayType(DataTypes.IntegerType), false),

                    DataTypes.createStructField("bbox_partitioning", BUCKET_SCHEMA, true)
            });

    public Map<String, Object> parse(Geometry geometry) {
        return  ParseGeometryForNFSP.parseGeometry(geometry);
    }
    ...
    public Row geometryToRow(Geometry geometry) {
        ...
        Map<String, Object> geom =
                ParseGeometryForNFSP.parseGeometry(
                        geometry
                );
    ...
    public Geometry geometryToJTS(Row geoRow) {
        return NFSPDecoder.geometryToJTS(geoRow);
    }
    ...
    public void registerDecode() {
        ...
        spark.udf().register(
                "decodeGeometry",
                (Row geoRow) -> NFSPDecoder.geometryToJTS(geoRow),
                GeometryUDT$.MODULE$
        );
    }
